...
def export_pdf(filename, baseline, post_total, reduction, selected_actions, logo_image, fig1, fig2, lang_code):
    # Save charts as images via kaleido
    ...
    # Prepare temp files for FPDF image()
    ...
    if logo_image is not None:
        pdf.image(logo_image, x=160, y=8, w=40)

    pdf.set_font("Arial", "B", 16)
    title = LANGS[lang_code]["title"]
    ...
    pdf.cell(0, 8, f"Baseline CO2: {baseline['Total CO2 (tons)']:.1f} t/yr", ln=True)
    pdf.cell(0, 8, f"Post-action CO2: {post_total:.1f} t/yr", ln=True)
    pdf.cell(0, 8, f"Potential Reduction: {reduction:.1f} t/yr", ln=True)
    pdf.ln(4)
    ...
    pdf.ln(6)
    pdf.set_font("Arial", "B", 12); pdf.cell(0, 8, "Emissions (Before vs After):", ln=True)
    pdf.image(img1_path, x=10, y=None, w=180)
    pdf.ln(2)
    pdf.set_font("Arial", "B", 12); pdf.cell(0, 8, "Baseline Breakdown:", ln=True)
